...
def iter_dataset(
    ...
    if ds_name in ["mtbench", "mt-bench", "mt_bench"]:
        ...
        # ---------------- FLASK-Hard (HF OR local jsonl) ----------------
    if ds_name in ["flask-hard", "flask_hard", "flaskhard"]:
        # Keep original behavior by default (HF loader), but support local jsonl path
        # via dataset_hf_path if it points to a .jsonl file.
        hf_path = dataset_hf_path or "tatsu-lab/flask"  # placeholder default
        ...
        # NEW: local jsonl support (only for flask_hard preset, minimal change)
        if isinstance(hf_path, str) and hf_path.endswith(".jsonl") and os.path.exists(hf_path):
            count = 0
        ...
        # Otherwise: original HF path behavior unchanged
