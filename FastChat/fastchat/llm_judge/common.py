...
def run_judge_single(question, answer, judge, ref_answer, multi_turn=False):
    ...
    if os.getenv("OPENROUTER_API_KEY") or (model in OPENAI_MODEL_LIST):
...
def chat_completion_openai(model, conv, temperature, max_tokens, api_dict=None):
    """
    Judge path for OpenAI-compatible chat completion.

    - If OPENROUTER_API_KEY is set: use OpenRouter via OpenAI SDK (v1).
    - Else: fall back to legacy openai.ChatCompletion (original behavior).
    ...
    for _ in range(API_MAX_RETRY):
        try:
            # Prefer OpenRouter if available (your case)
            if os.getenv("OPENROUTER_API_KEY"):
                ...
                resp = client.chat.completions.create(
                    ...
                    extra_headers={
                        ...
                        "X-Title": os.getenv("OPENROUTER_X_TITLE", "fastchat-judge"),
                    },
                )
                output = (resp.choices[0].message.content or "").strip()
                break

            # Fallback: original OpenAI legacy path
            if api_dict is not None:
                openai.api_base = api_dict["api_base"]
                openai.api_key = api_dict["api_key"]
            else:
                # If you ever use real OpenAI, keep OPENAI_API_KEY set
